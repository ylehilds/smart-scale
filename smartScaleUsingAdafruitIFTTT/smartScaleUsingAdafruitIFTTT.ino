...
#define THRESHOLD 10      // Measures only if the weight is greater than 2 kg. Convert this value to pounds if you're not using SI units.
#define MINWEIGHT 100      // Measures only if the weight is greater than 2 kg. Convert this value to pounds if you're not using SI units.
#define MAXWEIGHT 400  // Restart averaging if the weight changes more than 0.5 kg.
  ...
  if (weight > MINWEIGHT && weight < MAXWEIGHT) { // Takes measures if the weight is greater than the threshold
    ...
    for (int i = 0; i < NUM_MEASUREMENTS; i++) {  // Takes several measurements
      ...
      if (abs(weight - weight0) > THRESHOLD) {
        //ht1632c.print("still");  // Display on LED matrix
        avgweight = 0;
        i = -1;
        weight0 = weight;
      }
